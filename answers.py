a1aa=['DET', 'NOUN', 'ADJ', 'VERB', 'ADP', '.', 'ADV', 'CONJ', 'PRT', 'PRON', 'NUM', 'X']
a1b=2649
a1c=12.061525956381585
a1d='function'
a2a=13
a2b=2.4630366660416674
a3c=2.883894963080882
a3d='DET'
a4a3=0.8941143860297515
a4b1=[('the', 'DET'), ('elf', 'NOUN'), ('was', 'VERB'), ('on', 'ADP'), ('the', 'DET'), ('shelf', '.')]
a4b2=[('the', 'DET'), ('elf', 'NOUN'), ('was', 'VERB'), ('on', 'ADP'), ('the', 'DET'), ('shelf', 'NOUN')]
a4b3='The tagger tagged this example incorrectly as "shelf" did not appear\nin any sentences in the training set. This ultimately means the tagger sets the base\nprobabilities '
a4c=60.73342283709656
a4d=70.21922688308214
a4e=['DET', 'NOUN', 'ADP', 'DET', 'NOUN', 'VERB', 'ADV']
a5t0=0.558440358495796
a5tk=0.6090732698882011
a5b='1) Additional unlabelled data would have helped as the word "he" did not appear\nin the labelled data, however, it appeared multiple times in the unlabelled data.\n2) Unlike T_0, T_k mislabels "all" as a VERB when it is actually a PRT. Hard EM\nhurts in that case as the unsupervised tagging must have incorrectly tagged the word\n"all" resulting in the T_k model to perform worse when classifying that word than\nbefore. This is the risk of using such an unsupervised technique for training.'
a6='You can build a POS tagger with smoothing so that it can still\nhandle words not included in the training set. Thus a lexical\nword not included in the training set will still be naively\nhandled by the POS tagger model.\nIf optimized properly the smoothed model should perform better\nthan the original parser, however, we must note if the training\nset misses too many samples found in the test set it can\nresult in oversmoothing and ultimately takes away too much\nprobability mass from seen events.'
a7='Not all corpora use the same set of POS tags. The Brown tagset\nwas converted to the universal one to make our train set compareable\nto samples from other corpora, otherwise we could have significant\ncomplications when equating equivalent tags from different tagsets.\nThe Brown tagset uses 31 tags in contrast to the universal tagset\nwhich uses 12. This increase in tags would add a notable amount of\ncomplexity to the model and without sufficient data will most likely\nresult in poor generalisation.'
a4full_vit=[[26.782208935674642, 27.175612695180405, 26.83747232781355, 25.69387747486122, 26.069930312266184, 25.33122669020975, 26.602772790690192, 26.7617409338053, 6.7157647918435055, 26.154747513228394, 28.346102791874564, 27.06281489027102], [34.01526253042612, 26.139704758753695, 34.54383920395947, 32.31894404945449, 34.25048137679512, 36.22833529220789, 27.617716596488677, 37.174460980126284, 34.96569792142533, 33.2167214529358, 30.719087535156927, 43.877716268423896], [52.96761801645391, 52.61690686578734, 53.24942252459456, 54.72356597513747, 52.53968502182914, 34.012187579823376, 51.358458806319945, 52.70225028680407, 55.14157828038837, 53.01419324341059, 54.12151433519213, 54.72321304758704], [64.0217433334945, 58.59624615156186, 64.37034195590547, 61.74080618135381, 67.52308363114038, 64.95730590909109, 44.11359964060699, 60.60376213192809, 62.6503191185711, 64.14971027238595, 61.85439770355395, 60.37766863378029]]
a4full_bp=[['PRON', 'PRON', 'PRON', 'PRON', 'PRON', 'PRON', 'PRON', 'PRON', 'PRON', 'PRON', 'PRON', 'PRON'], ['NOUN', 'ADJ', 'NOUN', 'NOUN', 'ADJ', 'VERB', 'ADJ', 'ADJ', 'NOUN', 'ADJ', 'NOUN', 'ADJ'], ['DET', 'DET', 'DET', 'DET', 'DET', 'DET', 'DET', 'DET', 'DET', 'DET', 'DET', 'DET']]
